...
// Ottengo la tabella e una variabile chiave direttamente dal path
  ...
  // In base al metodo HTTP selezionato effettuerà una query diversa
    switch ($metodo_HTTP) {
      case 'GET':
        $query = "SELECT * FROM `$tabella`".($key?" WHERE idordine=\"$key\"":'')."ORDER BY idordine DESC"; break;
      case 'PUT':
        $query = "UPDATE `$tabella` SET $set WHERE idordine=\"$key\""; break;
      case 'POST':
        $query = "INSERT INTO `$tabella` SET $set"; break;
      case 'DELETE':
        $query = "DELETE FROM `$tabella` WHERE idordine=\"$key\" "; break;
      }
